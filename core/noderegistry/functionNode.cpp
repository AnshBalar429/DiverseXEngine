...
FunctionNode::FunctionNode(Scene *scene_, const string &title, vector<std::string> input_size, vector<std::string> output_size)
...
bool FunctionNode::deserialize(const QJsonObject &data, unordered_map<string, uintptr_t> &hashmap) {
    ...
    for (auto i : in) {
        ...
        auto label = new_i.value("label").toString().toStdString();
        auto *sok = new SocketNode(this, new_i.value("index").toInt(), pos, label);
    ...
    for (auto o : out) {
        ...
        auto label = new_o.value("label").toString().toStdString();
        auto *sok = new SocketNode(this, new_o.value("index").toInt(), pos, label);
...
AddNode::AddNode(Scene *scene_, const string &title, vector<std::string> input_size, vector<std::string> output_size)
...
SubNode::SubNode(Scene *scene_, const string &title, vector<std::string> input_size, vector<std::string> output_size)
    :FunctionNode(scene_, title, input_size, output_size){
    vals = 0;
}
...
MulNode::MulNode(Scene *scene_, const string &title, vector<std::string> input_size, vector<std::string> output_size)
    :FunctionNode(scene_, title, input_size, output_size){
    vals = 0;
}
...
DivNode::DivNode(Scene *scene_, const string &title, vector<std::string> input_size, vector<std::string> output_size)
    :FunctionNode(scene_, title, input_size, output_size) {
    vals = 0;
}
